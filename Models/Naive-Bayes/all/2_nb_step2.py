...
def read_probs():
    ...
    with open("sample_prob1.txt", "r") as f:
        lines = f.readlines()
    for line in lines:
        line = line.strip()
...
for line in sys.stdin:
    ...
    click = float(fields[0].replace('"',' ').split(',')[2].replace('\''," "))
    impression = float(fields[0].replace('"',' ').split(',')[3].replace('\''," "))
    simid = float(fields[1])
    adid = float(fields[0].replace('"',' ').split(',')[5].replace('\''," "))
    depid  = float(fields[0].replace('"',' ').split(',')[7].replace('\''," "))
    posid = float(fields[0].replace('"',' ').split(',')[8].replace('\''," "))

    temp1 = get_prob_from_dict('query_title', "{0:.5f}".format(simid))
    temp4 = get_prob_from_dict('ad_id', adid)
    temp2 = get_prob_from_dict('depth', depid)
    temp3 = get_prob_from_dict('position', posid)

    t = float(temp1[0]*temp2[0]*temp3[0]*temp4[0])
    f = float(temp1[1]*temp2[1]*temp3[1]*temp4[1])
    ...
    print "%s\t%s\t%s\t%s\t%s" %('all', adid, output, click, impression)
