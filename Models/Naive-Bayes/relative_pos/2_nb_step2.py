...
for line in sys.stdin:
    line = line.strip()
    fields = line.split('\t')
    click = float(fields[2])
    impression = float(fields[3])
    adid = float(fields[1])
    #depid = fields [1]
    #posid = fields [2]
    temp1 = get_prob_from_dict('position_depth', "{0:.5f}".format(adid))
    ...
    print "%s\t%s\t%s\t%s\t%s" %('position_depth', adid, output, click, impression)
